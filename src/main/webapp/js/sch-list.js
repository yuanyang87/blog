        ...
        success:function(data){
            ...
            for (var i = 0; i < data.length; i++) { //遍历json数据列
                ...
                li += '<tr><td style="display: none" id="myId">'
                    +data[i].courseId+
                    '</td></td><td class="post-title"><a href="javascript:void(0)">'
                    +data[i].courseName+
                    '</td><td>发布</td><td>'
                    +data[i].courseTime+
...
$(function() {
    $(document).on('click','#del',function(){
        ...
        $.ajax({
            type:"DELETE",
            url:"/blog/management/deleteCourse/"+myId,
            success:function() {
                alert("删除成功！");
    ...
    $(document).on('click','#edit',function() {
        ...
        $.ajax({
            type:"GET",
            url:"/blog/management/findCourse/"+myId,
            success:function(data){
                var data = data.data;
                //console.log(data);
                $('#articel-title').val(data.courseName);
                //console.log(data.articleTitle);
                var content = data.courseContent;
